...
void CRoutine_FTtoT3::Init(float image_scale)
{
...
void CRoutine_FTtoT3::FTtoT3(cl_mem ft_loc, cl_mem data_phasor, cl_mem uv_points, cl_mem data_sign, int n_t3, cl_mem output)
{
	int err = 0;
	...
	err |= clSetKernelArg(mKernels[0], 4, sizeof(cl_mem), output);      // Output is stored on the GPU.
	COpenCL::CheckOCLError("Failed to set ft_to_t3 kernel arguments.", err);
